...
def extracting(state: IncidentAgentState):
    """
    Extracts alerts from Redis queue, deduplicates them, and determines if escalation is needed.
    """
    try:
        r_client.ping()
...
def summarize_stix_bundle(bundle_objects):
    ...
    Groups events, removing noise like unique timestamps.
    """
    event_counter = {}
    rel_counter = {}
    entities = set() 
...
def context_aggregator(state: IncidentAgentState):
    """
    Builds historical context for the target IPs by fetching previous alerts and STIX conversion.
    """
...
def hunter_agent(state: IncidentAgentState):
    """
    Acts as the L2 SOC Threat Hunter attempting to build an attack vector.
    """
...
def skeptic_agent(state: IncidentAgentState):
    """
    Acts as the L2 SOC Validation Analyst looking for False Positives.
    """
...
def judge_agent(state: IncidentAgentState):
    """
    The L3 Judge agent that determines the final verdict based on Hunter, Skeptic, and Neo4j context.
    """
